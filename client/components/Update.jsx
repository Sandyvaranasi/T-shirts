...
function submitHandler(){
  ...
  api.put(`/product/${params.id}`,formData,{
    headers:{
...
<input type="text" className="form-control" id="exampleFormControlInput1" placeholder="Fill sizes if any..." onChange={e=>setSizes(e.target.value)}/>
...
<div className="mb-3">
<label htmlFor="exampleFormControlInput1" className="form-label">Available</label>
<button type="radio" className="form-control m-1" onClick={e=>setAvailability(true)}>Yes</button>
<button type="radio" className="form-control m-1" onClick={e=>setAvailability(false)}>No</button>
</div> 
  <div className="form-group">
    <button className='bg-success mt-2 w-100' onClick={submitHandler} >Update Info</button>
    <Link to='/products'>Do not want to update?? Return to products Now!!!</Link>
  </div>
  </div>
  )
}
